...
public class GameController_multiThread : GameController {
    ...
    private static bool initRender = false;
    private static bool pollRunning = false;
    ...
    void Start()
    {
        ...
        updateSem = new Semaphore(1, 1);

        pollRunning = true;
        pollThrd = new Thread(pollThrdFunc);
        pollThrd.Start();
    ...
    */
    private void pollThrdFunc()
    {
        while (pollRunning)
        {
            if (currentClient.Poll() == 1)
            {
                if (!dataReady)
                {
                    updateSem.WaitOne();
